...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Practica5. Modelado jerarquico. Mendez Galicia Axel Gael", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Dedo4A
		model = glm::translate(modelTemp2, glm::vec3(0.25f, -0.35f, -0.375f));
		model = glm::rotate(model, glm::radians(dedo4A), glm::vec3(0.0f, 0.1f, 0.0f));
		modelTemp = model = glm::translate(model, glm::vec3(0.5f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(1.0f, 0.3f, 0.25f));
		color = glm::vec3(0.0f, 1.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		...
		//Dedo4B
		model = glm::translate(modelTemp, glm::vec3(0.5f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(dedo4B), glm::vec3(0.0f, 0.1f, 0.0f));
		modelTemp = model = glm::translate(model, glm::vec3(0.25f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.5f, 0.3f, 0.25f));
		color = glm::vec3(0.5f, 1.0f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
 ...
 void Inputs(GLFWwindow *window) {
	 ...
	 if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS && codo > -90.0f)
		 codo -= 0.014f;
	 // Muñeca
	 if (glfwGetKey(window, GLFW_KEY_Y) == GLFW_PRESS && muñeca < 25.0f)
		 muñeca += 0.04f;
	 if (glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS && muñeca > -25.0f)
		 muñeca -= 0.04f;
	 // Dedos A
